...
import HtmlIcon from "@/app/assets/svg/html";
...
import { getDirectoryName } from "@/app/helper";
...
const Edit: React.FC<EditInterface> = ({ params }: any) => {
  ...
  const changeStatus = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      let response = await api({
        url: "/game/edit",
        data: { _id: params.slug, is_under_review: event.target.checked },
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${getCookie("auth-token")}`,
        },
  ...
  const setState = (data: any) => {
    const normalizedPath = data.game_url.replace(/\\/g, "/");
    ...
    entryFile = fileName;
    showPosters = data.poster;
    description = data.description;
    underReview = data.is_under_review;
    projectName = getDirectoryName(data.game_url);
    selectedPoster = [];
    setFiles(files);
    setName(name);
    setTitle(title);
      ...
      <div className="grid md:grid-cols-2 gap-4 px-8 mt-5">
        ...
        <div className="p-6 border-[1px] border-black/20 dark:border-dark-primary-darkBlue/10">
          ...
          <div className="mt-5 flex items-center">
            <Switch checked={underReview} onChange={changeStatus} />
